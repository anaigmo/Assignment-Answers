...
# The criteria to accept a best hit is an e-value of 1e-10 and an overlap length of at least half the length
# of the query sequence. I have chosen those values according to what I have read in the literature. The
# e-value is quite tough, but above 1e-05 it seems it is not robust enough, and under 1e-10 are considered
# closed related sequences. Regarding the overlapping length, it's the most common value I've seen. 70-75%
# of the length of the sequence could be also used, but since the e-value is strict, I decided no to be so in
# ovelapping length. 
...
def find_reciprocal_best_hits(db1, db2)
  ...
  query_db1.each do |seq|  # looks for best hit in factory db for each sequence in query_db
    ...
    # Restrictions to alignments: evalue under 1e-10 and alignment length over half the length of query seq
    next unless report.hits[0] and report.hits[0].evalue < 1e-10 and report.hits[0].overlap > seq.length/2
